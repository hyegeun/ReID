...
# Main interface
cpdef evaluate_cy(distmat, q_pids, g_pids, q_camids, g_camids, q_frames, g_frames, max_rank, use_metric_cuhk03=False):
    ...
    return eval_market1501_cy(distmat, q_pids, g_pids, q_camids, g_camids, q_frames, g_frames, max_rank)
...
cpdef eval_market1501_cy(float[:,:] distmat, long[:] q_pids, long[:]g_pids,
                         long[:]q_camids, long[:]g_camids, long[:] q_frames, long[:] g_frames, long max_rank):
    ...
    for q_idx in range(num_q):
        ...
        for g_idx in range(num_g):
            if (g_pids[order[g_idx]] != q_pid) or (g_camids[order[g_idx]] != q_camid):
                ...
                query_gallery.append([q_idx, q_pid, q_camid+1, q_frame, g_pids[order[g_idx]], g_camids[order[g_idx]]+1, g_frames[order[g_idx]], (2-row_dist[order[g_idx]])/2, matches[g_idx]])
        ...
        query_ap.append([q_idx, q_pid, q_camid+1, q_frame, tmp_cmc_sum / num_rel]) # apap 리스트에 query id, query camid, average precision 추가 
